from typing import Any, Tuple, List

import cv2
import matplotlib
import numpy as np
import torch
from ultralytics import YOLO
from metric_depth.depth_anything_v2.dpt import DepthAnythingV2


class Perception(torch.nn.Module):
    YOLO_CLASS_MAP = {
        "person": 0,
        "bicycle": 1,
        "car": 2,
        "motorcycle": 3,
        "banana": 46,  # Example class for detection
        # Add more classes if needed
    }

    def __init__(self, 
                 depth_model: str = "vitl",
                 depth_input_size: int = 518,
                 yolo_model: str = "yolo11l",
                 device: str = "cuda",
                 side_ratio: float = 0.25,
                 safe_distance: float = 0.5,
                 beta: float = 0.6,
                 visualize: bool = False):
        super().__init__()

        self.depth_input_size = depth_input_size
        self.side_ratio = side_ratio
        self.safe_distance = safe_distance
        self.beta = beta
        self.visualize = visualize
        self.depth_anything = self.load_model(depth_model, device)
        self.yolo_model = YOLO(f"./checkpoints/{yolo_model}.pt")

        if self.visualize:
            self.cmap = matplotlib.colormaps.get_cmap("Spectral_r")
            cv2.namedWindow("Depth Anything V2", cv2.WINDOW_NORMAL)
            cv2.namedWindow("YOLO", cv2.WINDOW_NORMAL)

    def load_model(self, encoder, device) -> DepthAnythingV2:
        model_configs = {
            "vits": {"encoder": "vits", "features": 64, "out_channels": [48, 96, 192, 384]},
            "vitb": {"encoder": "vitb", "features": 128, "out_channels": [96, 192, 384, 768]},
            "vitl": {"encoder": "vitl", "features": 256, "out_channels": [256, 512, 1024, 1024]},
        }
        depth_anything = DepthAnythingV2(**{**model_configs[encoder], "max_depth": 10})
        depth_anything.load_state_dict(torch.load(f"./checkpoints/depth_anything_v2_metric_hypersim_{encoder}.pth", map_location="cpu"))
        return depth_anything.to(device).eval()

    def get_depth_image(self, raw_frame: np.ndarray) -> torch.Tensor:
        image, (h, w) = self.depth_anything.image2tensor(raw_frame, self.depth_input_size)
        depth: torch.Tensor = self.depth_anything.forward(image)
        depth = torch.nn.functional.interpolate(depth[:, None], (h, w), mode="bilinear", align_corners=True)[0, 0]
        return depth

    def get_depth_image_visual(self, depth_frame: torch.Tensor, grayscale: bool = False) -> np.ndarray:
        depth_img = depth_frame.detach().cpu().numpy()
        if grayscale:
            depth_img = np.clip(depth_img * 100.0, 0, 255).astype(np.uint8)
            depth_img = np.repeat(depth_img[..., np.newaxis], 3, axis=-1)
        else:
            depth_img = (depth_img - depth_img.min()) / (depth_img.max() - depth_img.min())
            depth_img = (self.cmap(depth_img)[:, :, :3] * 255)[:, :, ::-1].astype(np.uint8)
        return depth_img

    def avoid_obstacles_planner(self, depth_frame: torch.Tensor) -> List[Tuple[float, float, float]]:
        height, width = depth_frame.shape[:2]
        side_width = int(width * self.side_ratio)
        obstacles = []

        # Left side
        left_side = depth_frame[:, :side_width]
        if torch.min(left_side) < self.safe_distance:
            obstacles.append((-side_width / 2, 0, self.safe_distance))

        # Right side
        right_side = depth_frame[:, width - side_width:]
        if torch.min(right_side) < self.safe_distance:
            obstacles.append((side_width / 2, 0, self.safe_distance))

        # Center
        center = depth_frame[:, side_width:width - side_width]
        if torch.min(center) < self.safe_distance:
            obstacles.append((0, 0, self.safe_distance))

        return obstacles

    def rrt_planner(self, start: Tuple[float, float], goal: Tuple[float, float], obstacles: List[Tuple[float, float, float]], max_iter: int = 1000, step_size: float = 0.2) -> List[Tuple[float, float]]:
        import random

        def distance(p1, p2):
            return ((p1[0] - p2[0]) ** 2 + (p1[1] - p2[1]) ** 2) ** 0.5

        def is_collision_free(node, obstacles):
            for ox, oy, radius in obstacles:
                if distance(node, (ox, oy)) <= radius:
                    return False
            return True

        def nearest_node(tree, point):
            return min(tree, key=lambda node: distance(node, point))

        tree = {start: None}
        nodes = [start]

        for _ in range(max_iter):
            rand_point = (random.uniform(-1, 1), random.uniform(-1, 1))
            nearest = nearest_node(nodes, rand_point)

            direction = ((rand_point[0] - nearest[0]) / distance(rand_point, nearest),
                         (rand_point[1] - nearest[1]) / distance(rand_point, nearest))
            new_node = (nearest[0] + direction[0] * step_size, nearest[1] + direction[1] * step_size)

            if is_collision_free(new_node, obstacles):
                nodes.append(new_node)
                tree[new_node] = nearest
                if distance(new_node, goal) <= step_size:
                    path = [goal]
                    while new_node is not None:
                        path.append(new_node)
                        new_node = tree[new_node]
                    return path[::-1]
        return []

    def show_debug_view(self, depth_frame: torch.Tensor, yolo_results: list[Any], obstacles: List[Tuple[float, float, float]]):
        depth_img = self.get_depth_image_visual(depth_frame)
        for ox, oy, radius in obstacles:
            cv2.circle(depth_img, (int(ox), int(oy)), int(radius * 100), (0, 0, 255), 2)

        annotated_frame = yolo_results[0].plot()
        cv2.imshow("Depth Anything V2", depth_img)
        cv2.imshow("YOLO", annotated_frame)
        cv2.waitKey(1)

    def forward(self, raw_frame: np.ndarray) -> torch.Tensor:
        depth_frame = self.get_depth_image(raw_frame)
        yolo_results = self.yolo_model.predict(raw_frame, conf=0.2, classes=[self.YOLO_CLASS_MAP["banana"]], verbose=False)

        obstacles = self.avoid_obstacles_planner(depth_frame)
        start = (0, 0)
        goal = (0.5, 0)  # Default goal: move forward

        if yolo_results:
            boxes = yolo_results[0].boxes
            if boxes:
                highest_idx = torch.argmax(boxes.conf)
                xywhn = boxes[highest_idx].xywhn
                goal = (0.5, xywhn[0, 0] - 0.5)

        path = self.rrt_planner(start, goal, obstacles)
        next_step = path[1] if len(path) > 1 else start

        if self.visualize:
            self.show_debug_view(depth_frame, yolo_results, obstacles)

        return torch.tensor([next_step[0], next_step[1], 0])


if __name__ == "__main__":
    depth_model = "vits"
    depth_input_size = 518
    yolo_model = "yolo11l"
    device = "cuda" if torch.cuda.is_available() else "cpu"

    model = Perception(depth_model=depth_model, depth_input_size=depth_input_size, yolo_model=yolo_model, device=device, visualize=True)

    video_source = 0
    raw_video = cv2.VideoCapture(video_source)
    if not raw_video.isOpened():
        print("Error: Could not open video source.")
        exit()

    while True:
        ret, raw_frame = raw_video.read()
        if not ret:
            print("Error: Could not read frame.")
            break

        goal = model.forward(raw_frame)
        print(goal)

    raw_video.release()
    cv2.destroyAllWindows()
